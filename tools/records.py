...
def _rows(board, key_hi, key_lo, season_label):
    # No inline styles, no emoji: meaning rides on stylesheet classes.
    body = ""
    for i, r in enumerate(board, 1):
        hi = f'{r["highest"]["score"]:.1f} <span class="muted small">' \
             f'W{r["highest"]["week"]} {r["highest"]["year"]}</span>' if r["highest"] else "—"
        lo = f'{r["lowest"]["score"]:.1f} <span class="muted small">' \
             f'W{r["lowest"]["week"]} {r["lowest"]["year"]}</span>' if r["lowest"] else "—"
        body += (
            f'<tr><td><span class="rank-num">{i}</span></td>'
            f'<td class="strong">{r["owner"]}</td>'
            f'<td class="pos strong">{r[key_hi]}</td>'
            f'<td class="neg strong">{r[key_lo]}</td>'
...
def _weekly_block(data):
    ...
    if played_2026:
        ...
        cur = (
            f'<h3 class="sub-title">{data["season"]} — so far</h3>'
    ...
    return (
        ...
        '  <span class="eyebrow">Weekly Scoring</span>'
